...
```mysql
(select * from stuinfo having sex='male' order by age desc limit 1000) union all (select * from stuinfo having sex='female' order by age asc limit 1000);
```
...
```mysql
select * from stuinfo a left join stumarks b on a.sid=b.stuno;
...
```mysql
select * from stuinfo a right join stumarks b on a.sid=b.stuno;
...
2.显示学生参加数学考试的地区
select a.city,count(b.math) c from stuinfo a left join stumarks b using(sid) group by city having c>0;

select distinct a.city from stuinfo a right join stumarks b using(sid) where math is not null;
3.显示男生人数和女生人数
select a.sex,count(a.sex) from stuinfo a group by sex;

select sex,count(sex) from stuinfo where sex='male' union select sex,count(sex) from stuinfo where sex='female';

select sum(sex='male') 男,sum(sex='female') 女 from stuinfo;
4.显示每个地区的男生人数,女生人数,总人数
...
start transaction;
#语句
insert into stuinfo values(null,'百强','male',18,'黑龙江');
insert into stuinfo values(null,'百强1','male',18,'黑龙江');
insert into stuinfo values(null,'百强2','male',18,'黑龙江');
#不成功回滚
rollback;
#成功->提交
commit;
...
缺点:
1.更多的储存空间来储存索引字段名
2.使得insert,update,delete的速度变慢了(查询操作占用90%,cud操作加起来占用10%的操作还未必能达到)
```
...
fulltext key
只能在myisam表引擎下使用(创建的话都能创建,但是无效),使用like的时候提升效率.#  where nama like '%你%';
